...
export async function sendPasswordResetEmail(userEmail: string, resetToken: string) {
  ...
  await transporter.sendMail({
    from: process.env.SMTP_FROM,
    to: userEmail,
    subject: 'Reset Your Password - FinGuide',
    html: `
      <!DOCTYPE html>
      <html>
        <head>
...
export async function sendDataExport(userEmail: string, zipBuffer: Buffer) {
  await transporter.sendMail({
    from: process.env.SMTP_FROM,
    to: userEmail,
    subject: 'Your FinGuide Data Export',
    html: `
      <!DOCTYPE html>
      <html>
        <head>
    ...
    `,
    attachments: [{
      filename: `FinGuide export-${new Date().toISOString().split('T')[0]}.zip`,
      content: zipBuffer
    }]
  });
}
...
export async function sendVerificationEmail(userEmail: string, token: string) {
  ...
  await transporter.sendMail({
    from: process.env.SMTP_FROM,
    to: userEmail,
    subject: 'Verify Your Email - FinGuide',
    html: `
      <!DOCTYPE html>
      <html>
        <head>
