...
export default function SettingsPage() {
  ...
  const handleSettingChange = async (updates: Partial<UserSettings>) => {
    ...
    try {
      ...
      if (updates.preferences?.monthlyBudget !== undefined && isNaN(updates.preferences.monthlyBudget)) {
        throw new Error('Invalid monthly budget value');
      }
  
      const newSettings = {
        preferences: {
          currency: updates.preferences?.currency || settings.preferences.currency,
          language: updates.preferences?.language || settings.preferences.language,
          theme: updates.preferences?.theme || settings.preferences.theme,
          monthlyBudget: updates.preferences?.monthlyBudget || settings.preferences.monthlyBudget,
        },
        notifications: {
          email: updates.notifications?.email || settings.notifications.email,
          push: updates.notifications?.push || settings.notifications.push,
          sms: updates.notifications?.sms || settings.notifications.sms,
        },
        emailVerified: settings.emailVerified,
      };
  
      console.log("Sending payload:", newSettings); // Log the payload
      ...
      toast({
        title: "Error",
        description: error instanceof Error ? error.message : "Could not update your settings. Please try again later.",
        variant: "destructive",
      });
    } finally {
      setLoading(false);
